...
RSpec.describe "when i visit the item show endpoint" do
  ...
  describe "I see all the data for a single item" do
    ...
    it "Merchant items can return page 2, and defaults to 20 per page" do
      get "/api/v1/merchants/#{@merchant.id}/items", params: {page: 2}
      expect(response).to be_successful
      ...
      response[:data].each_with_index do |item, index|
        expect(item).to have_key(:id)
        expect(item[:id]).to be_a(String)
        expect(item[:id].to_i).to eq(@items[index + 20].id)

        expect(item).to have_key(:type)
        expect(item[:type]).to be_a(String)
        expect(item[:type].capitalize).to eq(@items[index + 20].class.to_s)
        ...
        expect(item[:attributes]).to have_key(:description)
        expect(item[:attributes][:description]).to be_a(String)
        expect(item[:attributes][:description]).to eq(@items[index + 20].description)

        expect(item[:attributes]).to have_key(:unit_price)
        expect(item[:attributes][:unit_price]).to be_a(Float)
        expect(item[:attributes][:unit_price]).to eq(@items[index + 20].unit_price)

        expect(item[:attributes]).to have_key(:merchant_id)
        expect(item[:attributes][:merchant_id]).to be_a(Integer)
        expect(item[:attributes][:merchant_id]).to eq(@items[index + 20].merchant_id)

        expect(item[:attributes]).to_not have_key(:created_at)
        expect(item[:attributes]).to_not have_key(:updated_at)
      end
    end
    it "Merchant items can return 2 per page, and defaults to page 1" do
      get "/api/v1/merchants/#{@merchant.id}/items", params: {per_page: 2}
      expect(response).to be_successful
